...
void ShowGraf(AVLNode* root, int level) {
    if (root == NULL) {
        return;
    }

    ShowGraf(root->right, level + 1);
...
//FUNÇÃO DE REMOÇÃO DE NOS DUPLICADOS
AVLNode* RemoveDuplicates(AVLNode* root) {
    if (root == NULL) {
        return NULL;
    }
    AVLNode* current = root;
    while (current != NULL) {
        AVLNode* duplicate = NULL;
        AVLNode* found_node = NULL;
        SearchNode(&root, current->key, &found_node);
        if (found_node != current) {
            duplicate = found_node;
        }
        if (duplicate != NULL) {
            root = RemoveNode(root, duplicate->key);
        }
...
//FUNÇÃO DE VERIFICAÇÃO DE BALANCEAMENTO PARA O MENU
int GetBalanceMenu(AVLNode *root){
...
//FUNÇÃO DE VERIFICAÇÃO DE ESTADO VAZIO DA ARVORE
